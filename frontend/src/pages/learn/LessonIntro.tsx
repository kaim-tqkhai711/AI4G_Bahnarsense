...
export function LessonIntro() {
    ...
    useEffect(() => {
        const loadLesson = async () => {
            ...
            try {
                ...
                // Tìm node intro_screen trong list steps
                const introStep = list.find((s: any) => s.type === 'intro_screen');
                setLessonData(introStep || null);
            } catch (err) {
    ...
    // Default values if JSON is missing something
    const introFields = lessonData.content?.intro || {};
    const topic = introFields.topic || lessonData.title || 'CHÀO HỎI';
    const totalLessonsInTopic = introFields.total_lessons || 4;
    const currentLesson = introFields.current_lesson || 1;
    const completedInTopic = 0; // Thay thế bằng dữ liệu thực tế từ user data sau nếu cần
    ...
    return (
        <div className="min-h-screen bg-stone-50 flex flex-col relative overflow-hidden">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center z-10 relative">
                ...
                {/* Mascot KơRai Area (Ảnh nhân vật người Ba Na dummy) */}
                <motion.div
                ...
                >
                    ...
                    {/* Nhân vật người Ba Na (Sử dụng CSS Art đơn giản biểu trưng cho người Ba Na hoặc SVG) */}
                    <div className="w-40 h-40 bg-orange-200 rounded-full border-8 border-white shadow-xl flex items-center justify-center relative overflow-hidden">
                        {/* Khăn quấn đầu thổ cẩm */}
                        <div className="absolute top-0 w-full h-12 bg-red-600 flex justify-around items-end pb-1 shadow-inner">
                            <div className="w-4 h-4 bg-yellow-400 rotate-45 mr-1" />
                            <div className="w-4 h-4 bg-yellow-400 rotate-45 mr-1" />
                            <div className="w-4 h-4 bg-yellow-400 rotate-45 mr-1" />
                            <div className="w-4 h-4 bg-yellow-400 rotate-45" />
                        </div>
                        {/* Mắt & Miệng */}
                        <div className="absolute top-16 left-10 w-3 h-3 bg-stone-900 rounded-full shadow-sm" />
                        <div className="absolute top-16 right-10 w-3 h-3 bg-stone-900 rounded-full shadow-sm" />
                        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-8 h-4 bg-red-400 rounded-b-full shadow-inner" />
                    </div>
                </motion.div>
                ...
                >
                    <h1 className="text-2xl font-black text-stone-800 mb-3 leading-tight">
                        ...
                        <span className="text-emerald-600">{topic.toUpperCase()}</span>
                    </h1>
                    ...
                    <div className="bg-emerald-50 rounded-xl p-4 mb-2 border border-emerald-100 text-left">
                        <div className="text-emerald-800 font-bold text-sm mb-1">Tiến trình của bạn:</div>
                        <ul className="text-sm font-medium text-emerald-700/80 mb-3 pl-4 list-disc marker:text-emerald-400">
                            <li>Có {totalLessonsInTopic} bài học trong chủ đề</li>
                            <li>Bạn đã học {completedInTopic}/{totalLessonsInTopic} bài học trong chủ đề</li>
                        </ul>
                        <div className="w-full h-2.5 bg-emerald-200/50 rounded-full overflow-hidden border border-emerald-100">
            ...
            >
                <button
                    onClick={() => navigate(`/lesson/${id}/interactive`)}
