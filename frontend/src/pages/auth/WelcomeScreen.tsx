...
export function WelcomeScreen() {
    ...
    const [emailStatus, setEmailStatus] = useState<'idle' | 'login' | 'signup'>('idle');
    ...
    // After OAuth redirect, Supabase puts tokens in the URL hash. Listen for session and sync with backend.
    useEffect(() => {
        let cancelled = false;

        const syncWithBackend = async (accessToken: string) => {
            if (cancelled || useUserStore.getState().isAuthenticated) return;
            setIsLoggingIn(true);
            try {
                await processLoginResponse(accessToken, 'oauth');
            } finally {
                if (!cancelled) setIsLoggingIn(false);
            }
        };

        // 1) Session may already be restored from hash – check once and after a short delay (client parses hash async)
        const checkSession = async () => {
            const { data: { session } } = await supabase.auth.getSession();
            if (session?.access_token) await syncWithBackend(session.access_token);
        };
        checkSession();
        const t = setTimeout(checkSession, 400);

        // 2) When auth state changes (e.g. session restored from redirect), sync
    ...
    const handleSocialLogin = async (providerName: 'google' | 'facebook') => {
        ...
        try {
            ...
            const { data: { session } } = await supabase.auth.getSession();
            if (session?.access_token) await processLoginResponse(session.access_token, providerName);
        } catch (error: unknown) {
        ...
        try {
            if (emailStatus === 'login') {
                ...
                if (data.session?.access_token) {
                    await processLoginResponse(data.session.access_token, 'email_login');
                }
            } else {
                const { data, error } = await supabase.auth.signUp({
                    email,
                    password,
                    options: { data: name ? { full_name: name, username: name } : undefined }
                });
                if (error) throw error;
                if (data.session?.access_token) {
                    await processLoginResponse(data.session.access_token, 'email_signup');
                } else {
            ...
            >
                ...
                {emailStatus !== 'idle' && (
                    ...
                    >
                        ...
                        <button
                            type="button"
                            onClick={() => setEmailStatus(emailStatus === 'signup' ? 'login' : 'signup')}
                            className="w-full text-stone-500 text-sm font-bold hover:text-stone-700"
                        >
                            {emailStatus === 'signup' ? 'Đã có tài khoản? Đăng nhập' : 'Chưa có tài khoản? Đăng ký'}
                        </button>
                    </motion.form>
                )}
